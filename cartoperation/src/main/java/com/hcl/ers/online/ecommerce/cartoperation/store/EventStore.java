...
public class EventStore {
	...
	public List<Event> getCartEvents(String cartId, long snapshotEventSequenceNumber) {
    	...
    	N1qlQuery query = N1qlQuery.parameterized("select "+ bucketName +".* from `"+ bucketName +"` where docType='CartEvent' and cartId=$cartId and eventNumber > $snapshotEventNumber order by eventNumber", values, params);
